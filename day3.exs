defmodule Day3 do
  def part1(input) do
    ...
    lines |> Enum.reduce({ List.duplicate(0, cols), 0 }, fn current, { ones, rows } ->
  ...
  def is_one(s) do
    if s == "1" do 1 else 0 end
  end

  def to_gamma_and_epsilon({ ones, rows }) do
    gamma = ones
    |> Enum.map(fn current -> if current * 2 <= rows do 0 else 1 end end)
    |> from_binary()
  ...
  def part2(input) do
    ...
    oxigen = lines
    |> recursive_filter(fn ones, rows -> if ones * 2 >= rows do 1 else 0 end end, 0)
    |> List.first()
    |> from_binary()
