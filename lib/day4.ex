defmodule Day4 do
  ...
  def mark_boards(boards, positions_to_mark) do
    ...
    |> Enum.reduce(boards, fn {board_id, y, x}, boards ->
      boards
      |> Kernel.update_in([Access.at!(board_id), Access.at!(y), Access.at!(x)], fn {value, _} ->
        {value, true}
      end)
    end)
  end
  ...
  def part2(input) do
    ...
    draw
    |> Enum.reduce_while({boards, remaining_board_ids, 0}, fn value,
                                                              {boards, remaining_board_ids, _} ->
      new_boards =
        case positions[value] do
          nil -> boards
          positions_to_mark -> mark_boards(boards, positions_to_mark)
      ...
      case game_over_2(new_boards, remaining_board_ids) do
        {[winner_board_id], []} ->
          {:halt, {new_boards, [], compute_score(Enum.at(new_boards, winner_board_id), value)}}

        {winner_board_ids, _} ->
          {:cont,
           {new_boards,
            MapSet.to_list(
              MapSet.difference(MapSet.new(remaining_board_ids), MapSet.new(winner_board_ids))
            ), 0}}
      end
    end)
    |> case do
